...
import be.c4j.ee.security.beans.metadata.ContextualLifecycle;
 ...
 * <p/>
 * <p>
 * The builder can read from an {@link javax.enterprise.inject.spi.AnnotatedType} and have any attribute
    ...
    public Bean<T> create() {
        if (!passivationCapable) {
            return new ImmutableBean<T>(beanClass, name, qualifiers, scope, stereotypes, types, alternative,
                    nullable, injectionPoints, toString, beanLifecycle);
        } else {
            throw new UnsupportedOperationException("Only non passivationCapable supported");
        }
