...
public class JWTTesterEC {
    ...
    public static void main(String[] args) throws IOException, ParseException, JOSEException {
        ...
        String token = createToken((ECKey) publicJWK, apiKey);

        String publicContent = readFile("public_EC.jwk");

        JWKSet jwkSet = JWKSet.parse(publicContent);

        JWK jwkForApiKey = jwkSet.getKeyByKeyId(apiKey);
        showTokenContent(apiKey, token, (ECKey) jwkForApiKey);

    }

    private static void showTokenContent(String apiKey, String token, ECKey jwkForApiKey) throws ParseException, JOSEException {
        // Parse token
        SignedJWT signedJWT = SignedJWT.parse(token);

        // CRtea verifier using the RSA key
        JWSVerifier verifier = new ECDSAVerifier(jwkForApiKey);
    ...
    private static String createToken(ECKey publicJWK, String apiKey) throws JOSEException {
        // Create  signer
        JWSSigner signer = new ECDSASigner(publicJWK);
        ...
        JWSHeader header = new JWSHeader.Builder(JWSAlgorithm.ES512).type(JOSEObjectType.JWT).keyID(apiKey).build();
    ...
    private static String readFile(String fileName) {
        InputStream privateKeys = JWTTesterEC.class.getClassLoader().getResourceAsStream(fileName);
