...
public class PermissionService {
    ...
    public List<NamedDomainPermission> getAllPermissions() {
        ...
        return convertPermissions(permissionList);
    }

    private List<NamedDomainPermission> convertPermissions(List<HRAppPermission> permissionList) {
        List<NamedDomainPermission> result = new ArrayList<NamedDomainPermission>();
        for (HRAppPermission permission : permissionList) {
            result.add(new NamedDomainPermission(permission.getName(), permission.getPermission()));
        }
        return result;
    }

    public Collection<NamedDomainPermission> getPermissionsForPrincipal(UserPrincipal primaryPrincipal) {
        List<HRAppPermission> permissions = new ArrayList<HRAppPermission>(database.getPermissions(primaryPrincipal.getId()));
        return convertPermissions(permissions);
    }
}
